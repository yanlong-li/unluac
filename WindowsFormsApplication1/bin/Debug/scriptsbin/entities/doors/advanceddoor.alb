AdvancedDoor = {
  ...
  Editor = {Icon = "Door.bmp", ShowBounds = 1},
  States = {
    "Opened",
    "Closed",
    "Destroyed"
...
function AdvancedDoor:CheckInitalAngle()
  ...
  if angle ~= 0 then
    if angle < 0 and 0 > self.openingrange or angle > 0 and 0 < self.openingrange then
...
function AdvancedDoor:Apply()
  ...
  ConstraintParams.frame0_inner = {
    x = 0,
    y = 90,
    z = 0
  }
  ConstraintParams.frame1_inner = {
    x = 0,
    y = 90,
    z = 0
  }
  if 0 < self.openingrange then
...
function AdvancedDoor:OnUpdate()
  if self.bUpdate == 1 then
    ...
    if self.angle.z == self.lastangle then
      ...
      if System.GetCurrTime() - self.lasttime > 5 then
        self.lasttime = 0
        self.bUpdate = 0
      end
    end
    ...
    elseif math.abs(self.angle.z - self.defaultangle) > 0.5 and self:GetState() ~= "Opened" then
      self:GotoState("Opened")
    elseif 0.5 > 360 - math.abs(self.angle.z - self.defaultangle) and self:GetState() ~= "Closed" then
      self:SetAngles(self.initialrot)
      self:Play(0)
      self:GotoState("Closed")
    end
    if self.Properties.Limits.fAutoCloseTime ~= 0 and self:GetState() == "Opened" and self.iAutoCloseTimer == nil then
...
function AdvancedDoor:AutoClose()
...
function AdvancedDoor:PhysicalizeThis(slot, bRigidBody)
  ...
  self:SetPhysicParams(PHYSICPARAM_VELOCITY, {
    v = {
      x = 0,
      y = 0,
      z = 0
...
function AdvancedDoor.Server:OnHit(hit)
  ...
  if hit.type == "melee" and NumberToBool(vul.bMelee) then
    ...
    damage = damage / 4
    if damage > 80 and self:GetState() == "Closed" then
      self:BreachDoor()
    end
  end
end
...
function AdvancedDoor:BreachDoor(user)
  if self.Properties.bIsBreachable == 0 or not self:CheckBreachDirection(user) then
    return
  end
  self:Activate(1)
  self.bUpdate = 1
...
function AdvancedDoor:CheckUseDirection(user)
  ...
  elseif limits.bOpenFromBack == 0 then
    return false
  end
  return true
end
