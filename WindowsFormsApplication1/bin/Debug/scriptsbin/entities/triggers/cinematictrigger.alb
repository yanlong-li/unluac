...
function CinematicTrigger:Event_Trigger(sender)
  ...
  if 0 < string.len(self.Properties.ScriptCommand) then
    dostring(self.Properties.ScriptCommand)
  end
  if 0 < string.len(self.Properties.Sequence) then
...
function CinematicTrigger:IsVisible()
  ...
  point3.y = p.y - dy
  point3.z = p.z - dz
  if System.IsPointVisible(point0) and System.IsPointVisible(point1) and System.IsPointVisible(point2) and System.IsPointVisible(point3) then
    point0.x = p.x - dx
    point0.y = p.y + dy
    point0.z = p.z + dz
    point1.x = p.x - dx
...
function CinematicTrigger:CheckZoom()
  if self.Properties.fZoomMinimum > 1 then
    local fov = System.GetViewCameraFov() / math.pi * 180
