Chick = {
  ...
  Properties = {
    ...
    Boid = {
      nCount = 10,
      object_Model = "game/objects/characters/animals/boids/mushroom/boid_mushroom_a.chrr",
      Size = 1,
      SizeRandom = 0,
    ...
    Sounds = {
      soundS0_cluck = "sounds/x2ambience:world2_animal:chick_cluck",
      soundS1_scared = "",
      soundS2_die = "",
      soundS3_pickup = "",
      soundS4_throw = ""
...
function Chick:OnSpawn()
  self:SetFlags(ENTITY_FLAG_CLIENT_ONLY, 0)
end
function Chick:OnInit()
  self.flock = 0
  self.currpos = {
    x = 0,
    y = 0,
...
function Chick:OnShutDown()
end
function Chick:GetFlockType()
  return Boids.FLOCK_CHICKENS
end
function Chick:GetSounds()
...
function Chick:OnPropertyChange()
  self:OnShutDown()
  if self.Properties.Options.bActivate == 1 then
    self:CreateFlock()
  end
end
function Chick:Event_Activate()
...
function Chick:Event_Deactivate()
  if self.flock ~= 0 then
    Boids.EnableFlock(self, 0)
  end
end
function Chick:OnProceedFadeArea(player, areaId, fadeCoeff)
  if self.flock ~= 0 then
...
Chick.FlowEvents = {
  Inputs = {
    Activate = {
      Chick.Event_Activate,
      "bool"
    },
    Deactivate = {
      Chick.Event_Deactivate,
      "bool"
    }
  },
