...
function Mine:OnReset()
  ...
  if self.Properties.Claymore.bIsClaymore ~= 0 or self.Properties.Options.bWaterMine == 0 then
  end
  self.health = 1
  self.ents = {}
  if self.Properties.bAdjustToTerrain == 1 and self.Properties.Claymore.bIsClaymore == 0 then
    local pos = {
      x = 0,
      y = 0,
      z = 0
    ...
    pos.z = System.GetTerrainElevation(self:GetPos()) - 0.01
    self:SetPos(pos)
  end
...
function Mine:Jump()
  ...
  local v0 = math.sqrt(19.62 * self.Properties.FrogMine.fJumpHeight)
  local force = 20 * v0
  local impulsepos = g_Vectors.v000
  CopyVector(impulsepos, self:GetPos())
  impulsepos.x = impulsepos.x + 0.005
  self:AddImpulse(-1, impulsepos, {
    x = 0,
    y = 0,
    z = 1
  }, force, 1)
  self:PlaySoundEvent("", g_Vectors.v000, g_Vectors.v010, SOUND_DEFAULT_3D, SOUND_SEMANTIC_MECHANIC_ENTITY)
...
function Mine:CheckEntities()
  ...
  for i, v in pairs(self.ents) do
    if v ~= nil then
      ...
      elseif v:IsEntityInside(self.id) then
        self:GotoState("Destroyed")
        break
      end
    end
