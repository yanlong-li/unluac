...
function Door:UpdateRotation(frameTime)
  ...
  if currAngles.x < 0 and goalAngles.x > 0 then
    currAngles.x = currAngles.x + 2 * math.pi
  end
  if currAngles.x > 0 and goalAngles.x < 0 then
    currAngles.x = currAngles.x - 2 * math.pi
  end
  if 0 > currAngles.y and 0 < goalAngles.y then
    currAngles.y = currAngles.y + 2 * math.pi
  end
  if 0 < currAngles.y and 0 > goalAngles.y then
    currAngles.y = currAngles.y - 2 * math.pi
  end
  if 0 > currAngles.z and 0 < goalAngles.z then
    currAngles.z = currAngles.z + 2 * math.pi
  end
  if 0 < currAngles.z and 0 > goalAngles.z then
    currAngles.z = currAngles.z - 2 * math.pi
  end
  local distSq = vecLenSq(vecSub(goalAngles, currAngles))
  if distSq < 1.0E-4 then
    self.rotationUpdate = nil
    if self.action == DOOR_CLOSE and not self.slideUpdate and self.portal then
      System.ActivatePortal(self:GetWorldPos(), 0, self.id)
    end
  end
end
function Door:UpdateSlide(frameTime)
  ...
  if distSq < 1.0E-4 then
    self.slideUpdate = nil
    if self.action == DOOR_CLOSE and not self.rotationUpdate and self.portal then
      System.ActivatePortal(self:GetWorldPos(), 0, self.id)
    end
  end
end
