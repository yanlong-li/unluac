...
function Hazard:AddScreenEffect(ent)
  if ent.actor:IsPlayer() then
    ...
    elseif dmg == 3 and self.fx.electricity.active ~= 1 then
...
function Hazard:RemoveScreenEffect(ent)
  if ent.actor:IsPlayer() then
    local dmg = self.dmg
    if self.fx[dmg] and self.fx[dmg].active == 1 then
...
Hazard.Server.Activated = {
  ...
  OnTimer = function(self, timerId, msec)
    if timerId == CHECK_TIMER and self.ents and table.getn(self.ents) > 0 then
      self:HandleEntities()
      self:SetTimer(CHECK_TIMER, 1000)
    end
  end,
