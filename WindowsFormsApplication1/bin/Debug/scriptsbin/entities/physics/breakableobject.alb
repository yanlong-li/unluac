...
function BreakableObject:OnReset()
  ...
  self:Physicalize(0, physType, {
    mass = props.fMass,
    density = props.fDensity
  })
  ...
  self:SetPhysicParams(PHYSICPARAM_PART_FLAGS, {
    partId = 0,
    mat_breakable = props.nBreakableType
  })
  local planes = props.PhysicsBreakable.GroundPlanes
  local plane = 0
  if planes.positiveX ~= 0 then
    self:SetGroundPlane(plane, 1, {
      x = 1,
      y = 0,
      z = 0
    }, planes.positiveX)
...
function BreakableObject:SetGroundPlane(index, axis, n, distance)
  ...
  local org = {
    x = c.x + n.x * size.x * (distance - 0.5),
    y = c.y + n.y * size.y * (distance - 0.5),
    z = c.z + n.z * size.z * (distance - 0.5)
  }
  self:SetPhysicParams(PHYSICPARAM_GROUND_PLANE, {
    origin = org,
    normal = n,
    plane_index = index
  })
